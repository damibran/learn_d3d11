...
cbuffer DirLightMatCB : register(b4)
{
    DirLightMat dir_light_mat;
};
...
vs_out VSMain(vs_in input)
{
    ...
    output.dirLightPos = mul(float4(output.worldPos, 1.), dir_light_mat.viewProj);
    return output;
}
...
Texture2D shadowmapT : register(t1);
...
float DirLightShadow(float4 pixLightPos)
{
...
float4 PSMain(vs_out input) : SV_TARGET
{
    ...
    float SMs = DirLightShadow(input.dirLightPos);
